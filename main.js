...
class SwissWeatherApi extends utils.Adapter {
	...
	async onReady() {
		...
		function getWeatherInfos() {
			self.log.info("Swiss-Weather-API: Start getWeatherInfos");
						...
						res.on("end", function () {
							...
							self.setObjectNotExists("CurrentForecast.current_day.values.icon-url", {
								...
								self.setState("CurrentForecast.current_day.values.icon-url", {
									val: "https://raw.githubusercontent.com/baerengraben/ioBroker.swiss-weather-api/master/img/weather-icons/png_64x64/" + icon + ".png",
									ack: true
								});
							});
		...
		async function doTheWork(proz){
			await getWeatherInfos();
			proz.stop();
		}

		// Ein Scheduled-Job wird nicht einfach nach nach Beendigung von onReady beendet:
		//
		...
		//
		// doTheWork ruft deswegen getWeatherInfos() mit await auf. Das sollte eigentlich sicherstellen, dass
		// das resultat von getWeatherInfos() abgewartet wird. Bin mir aber nicht sicher ob das auch die
		// vier http-calls abwartet...?
		doTheWork(self);
	}
}
